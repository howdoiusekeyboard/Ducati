...
export const clearAllUserData = async (userId: string): Promise<ClearDataResult> => {
  ...
  try {
    ...
    try {
      ...
      result.errors.push(
        `Failed to clear purchase history: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    try {
      ...
      result.errors.push(
        `Failed to clear financial profile: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    try {
      ...
      result.errors.push(
        `Failed to clear chat history: ${error instanceof Error ? error.message : String(error)}`
      );
    }
    ...
    try {
      ...
      result.errors.push(
        `Failed to clear pro mode analyses: ${error instanceof Error ? error.message : String(error)}`
      );
    }
