...
const getDecisionCriteria = (riskTolerance) => {
  ...
  keys.forEach((key, index) => {
    if (index < keys.length - 1) {
      ...
      adjustedCriteria[key].weight =
        Math.round((adjustedCriteria[key].weight / totalWeight) * 10000) / 10000;
...
const SCORING_FUNCTIONS = {
  ...
  frequencyOfUse: (frequency, itemType) => {
    ...
    if (itemType === 'consumable') {
      ...
      switch (frequency) {
        case 'Daily':
          return 6; // Daily food purchases might be expensive habit
        case 'Weekly':
          return 8; // Weekly treat is reasonable
        case 'Monthly':
          return 9; // Monthly dining out is very reasonable
        case 'Rarely':
          return 10; // Rare splurge is fine
        case 'One-time':
          return 8; // Single meal purchase is normal
        default:
          return 7;
      }
    } else {
  ...
  buyersRemorse: (cost, financialProfile, frequency, itemType) => {
    ...
    if (itemType !== 'consumable') {
      if (frequency === 'Daily')
        score += 2; // Daily use reduces remorse
 ...
 */
export const calculateDecisionScores = (
  itemName,
  cost,
  purpose,
  frequency,
 ...
 */
export const generateStructuredRecommendation = (
  decisionAnalysis,
  itemName,
  cost,
  alternative,
  ...
  const topNegative = Object.values(scores)
...
const getScoreExplanation = (criterionId, score, itemType) => {
  ...
  const defaultExplanations = {
    affordability: {
      ...
      medium: 'Manageable expense',
      low: 'Significant financial impact',
    },
    valueForMoney: {
      high: 'Excellent value proposition',
      medium: 'Fair market value',
      low: 'Overpriced compared to alternatives',
    },
    opportunityCost: {
      high: 'Minimal impact on other goals',
      medium: 'Some trade-offs required',
      low: 'Significant opportunity cost',
    },
    financialGoalAlignment: {
      high: 'Aligns well with financial goals',
      medium: 'Neutral impact on goals',
      low: 'May detract from financial goals',
    },
    necessity: { high: 'Essential item', medium: 'Useful but not critical', low: 'Luxury or want' },
    frequencyOfUse: {
      high: 'Will be used regularly',
      medium: 'Moderate usage expected',
      low: 'Limited usage anticipated',
    },
    longevity: {
      high: 'Durable and long-lasting',
      medium: 'Average lifespan',
      low: 'Consumable or short-lived',
    },
    emotionalValue: {
      high: 'High potential for satisfaction',
      medium: 'Some emotional benefit',
      low: 'Low emotional return',
    },
    socialFactors: {
      high: 'Purchase is internally motivated',
      medium: 'Some social influence',
      low: 'Likely driven by social pressure',
    },
    buyersRemorse: {
      high: 'Low risk of regret',
      medium: 'Some risk of regret',
      low: "High risk of buyer's remorse",
    },
    financialRisk: {
      high: 'Low risk to financial stability',
      medium: 'Moderate financial impact',
      low: 'High risk to financial health',
    },
    alternativeAvailability: {
      high: 'This is a good option',
      medium: 'Alternatives exist but are comparable',
      low: 'Better alternatives are likely available',
    },
  };
